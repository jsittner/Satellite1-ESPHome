...
bool Satellite1::transfer( uint8_t resource_id, uint8_t command, uint8_t* payload, uint8_t payload_len){
  ...
  do {
    ...
    this->disable();
    vTaskDelay(1);
    //ESP_LOGD( TAG, "In CMD: %x %x %x %x %x", send_recv_buf[0], send_recv_buf[1], send_recv_buf[2], send_recv_buf[3], send_recv_buf[4] );
  ...
  if( send_recv_buf[0] == DC_RESOURCE::CNTRL_ID && send_recv_buf[1] != DC_RET_STATUS::PAYLOAD_AVAILABLE ){
    ...
    uint8_t *arr = this->dc_status_register_;
    //ESP_LOGD( TAG, "New Status Register: %x %x %x %x", arr[0], arr[1], arr[2], arr[3] );
  }
  ...
  if( command & 0x80 ){
    ...
    do {
        ...
        this->disable();
        //ESP_LOGD(TAG, "RECEIVED: %x %x %x %x %x", send_recv_buf[0], send_recv_buf[1], send_recv_buf[2], send_recv_buf[3], send_recv_buf[4] );
        vTaskDelay(1);
    } while( send_recv_buf[0] == CONTROL_COMMAND_IGNORED_IN_DEVICE && attempts-- > 0 );
