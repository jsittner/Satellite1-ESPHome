...
esp_err_t AudioDecoder::start(media_player::MediaFileType media_file_type) {
  ...
  ESP_LOGD(TAG, "Starting AudioDecoder.");
...
AudioDecoderState AudioDecoder::decode(bool stop_gracefully) {
  ...
  if (this->potentially_failed_count_ > 10) {
    ESP_LOGD(TAG, "Potentially failed more than 10 times.");
  ...
  while (state == FileDecoderState::MORE_TO_PROCESS) {
    if (this->output_buffer_length_ > 0) {
      ...
      if ((this->input_buffer_length_ == 0) || ((this->potentially_failed_count_ > 0) && (bytes_read == 0))) {
        ESP_LOGD(TAG, "skip decoding: buffer: %d, failed: %d, to-read: %d read: %d avail: %d", this->input_buffer_length_,
        ...
        if(this->potentially_failed_count_ > 0){
          ESP_LOGD(TAG, "re-start decoding: buffer: %d, failed: %d, to-read: %d read: %d avail: %d", this->input_buffer_length_,
         this->potentially_failed_count_, bytes_to_read, bytes_read, this->input_ring_buffer_->available() ); 
        }
    ...
    if (state == FileDecoderState::POTENTIALLY_FAILED) {
      ESP_LOGD(TAG, "Decoder potentially failed");
...
FileDecoderState AudioDecoder::decode_flac_() {
  ...
  if (result == flac::FLAC_DECODER_ERROR_OUT_OF_DATA) {
    ...
    ESP_LOGD(TAG, "Bytes available in buffer: %d", this->input_buffer_length_ );
    ESP_LOGD(TAG, "Bytes read: %d", this->flac_decoder_->get_bytes_index() );
    ESP_LOGD(TAG, "Bytes left: %d", this->flac_decoder_->get_bytes_left() );
