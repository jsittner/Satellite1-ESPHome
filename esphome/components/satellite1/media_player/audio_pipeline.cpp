...
static const size_t FILE_RING_BUFFER_SIZE = 4 * FILE_BUFFER_SIZE;//64 * 1024;
  ...
  while (true) {
...

    {
      ...
      if (err != ESP_OK) {
        ...
        while (this_pipeline->raw_file_ring_buffer_.get()->available() < 2 * FILE_BUFFER_SIZE) {
        // If the maximum wait time is exceeded, exit the loop
        ...
        if ((xTaskGetTickCount() - xStartTime) >= pdMS_TO_TICKS(5000)) {
            break;
        }
        
        // Wait for a short delay before checking again
        vTaskDelay(pdMS_TO_TICKS(10)); // Delay 10ms (can be adjusted)
    }
...
void AudioPipeline::decode_task_(void *params) {
  ...
  while (true) {
...

    {
      ...
      ESP_LOGD(TAG, "Creating and starting new AudioDecoder");
